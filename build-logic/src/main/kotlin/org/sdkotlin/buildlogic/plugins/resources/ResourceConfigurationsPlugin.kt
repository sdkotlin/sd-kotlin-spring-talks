...
class ResourceConfigurationsPlugin : Plugin<Project> {
	...
	override fun apply(project: Project) {
		...
		extension.resourceConfigurations.get()
			.forEach { resourceConfiguration ->
				...
				val consumableConfigurationName =
					"${resourceConfiguration}Elements"
				...
				val dependencyHandlerExtensionName =
				...
				project.dependencies.extensions.add(
					...
					LibraryElementsAttributeDependencyCreationExtension(
						...
						libraryElementsAttributeValue = resourceConfiguration
					)
				)

				// Any files in "src/main/<resourceConfiguration>/" are resources for this
				// configuration.
				val resourcesDirectory: Directory =
					project.layout.projectDirectory.dir("src/main/$resourceConfiguration")
				...
				if (resourcesDirectory.asFile.exists()) {
					project.artifacts.add(consumableConfigurationName,
						resourcesDirectory) {
